...
public class MigrationService : IMigrationService
{
    ...
    public async Task<MigrationRecord?> CreateInvitationForUserAsync(MicrosoftIdentityUser user)
    {
        ...
        try
        {
            ...
            // Create invitation
            var invitation = new CiamInvitation
            {
                Email = user.Mail,
                FirstName = user.GivenName,
                LastName = user.Surname,
                ContactId = ExtractContactId(user),
                AdditionalAttributes = BuildAdditionalAttributes(user)
            };

            if (_configuration.SendInvitationEmails)
            {
                var invitationResponse = await _ciamService.SendInvitationAsync(invitation, accessToken);
                
                if (invitationResponse != null)
                {
                    migrationRecord.CiamUserId = invitationResponse.InvitationId;
                    migrationRecord.CiamUserName = user.UserPrincipalName;
                    migrationRecord.Status = MigrationStatus.Completed;
                    migrationRecord.MigratedAt = DateTime.UtcNow;
                    migrationRecord.ErrorMessage = $"Invitation sent. Expires: {invitationResponse.ExpiresAt}";

                    _logger.LogInformation($"Successfully sent invitation to {user.UserPrincipalName}. Invitation ID: {invitationResponse.InvitationId}");
                }
                else
                {
                    throw new InvalidOperationException("Failed to send invitation - no response received");
                }
            }
            else
            {
                // Just create user without sending invitation email
                var ciamUser = _userMappingService.MapToCiamUser(user);
    ...
    private async Task<MigrationRecord?> MigrateUserDirectAsync(MicrosoftIdentityUser user)
    {
        ...
        try
        {
            ...
            var isValid = await _ciamService.ValidateUserAsync(createdUser.Id, accessToken);
            if (!isValid)
            {
                throw new InvalidOperationException($"User validation failed for CIAM user ID {createdUser.Id}");
            }
